    ...
    <>
      ...
      <main className="bg-light min-vh-100 w-100 d-flex flex-column align-items-center py-2 px-3" style={{ backgroundColor: '#fffdf5', paddingTop: '3rem' }}>
        <div className="container-fluid position-relative" style={{ maxWidth: '1920px' }}>
          ...
          {/* Page Title */}
          <h1 className="fw-bold text-dark text-center mt-4 mb-4" style={{ fontSize: '36px', fontFamily: "'Poppins', sans-serif", color: '#2f2f3e' }}>
            Tentang Kami
          </h1>

          {/* Team Members Grid */}
          <div className="row row-cols-1 row-cols-md-2 row-cols-lg-2 g-3 mb-4 mt-2 justify-content-center">
            {teamMembers.map(member => (
              <div key={member.id} className="col">
                <div className="card h-100 shadow-sm border-0 rounded-lg text-center" style={{ padding: '10px' }}>
                  <img
                    src={member.photo}
                  ...
                  <div className="card-body p-0">
                    <h5 className="card-title fw-bold mb-1" style={{ fontSize: '1.1rem', color: '#2e2e2e' }}>{member.name}</h5>
                    <p className="card-text text-muted mb-1" style={{ fontSize: '1rem' }}>NIM: {member.nim}</p>
                    <p className="card-text text-secondary" style={{ fontSize: '0.8rem', fontStyle: 'italic' }}>{member.role}</p>
                  </div>
                </div>
              </div>
            ))}
